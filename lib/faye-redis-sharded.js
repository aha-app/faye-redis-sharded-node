...
try {
  sentry = require('../../../src/util/sentry');
} catch (e) {
  sentry = {
    logError: function () {}
  };
...
var Engine = function (server, options) {
  ...
  var shards = this._options.shards;
  ...
  function connectToShards(shards, isPrimary) {
    ...
    shards.forEach(function (shard) {
      ...
      var shardName = shard.shardName || (new URL(shard.url).host);
...
function connectRedis(connection, options, onRedisError){
  var client = redis.createClient(connection, options);
